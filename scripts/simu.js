...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/simu.ts");
/******/ })
/************************************************************************/
/******/ ({
...
function randomBoolean() {
    let outcome = Math.floor(Math.random() + 0.5);
    switch (outcome) {
        case 0:
            return false;
        case 1:
...
class Universe {
    constructor() {
        this.reproductiveAgeBegin = 15;
        this.reproductiveAgeEnd = 55;
        this.fightingAgeBegin = 20;
        this.fightingAgeEnd = 40;
        this.matingAgeBegin = 16;
        this.matingAgeEnd = 50;
        this.casualtiesBeforeRetreat = 0.3; /* [0..1] - Percentage of deaths before one party retreats */
        this.survivalByAge = [[1, 0.2], [10, 0.4], [14, 0.5], [20, 0.6], [35, 0.8],
...
class Tribe {
    constructor(universe) {
        this.individuals = [];
        this.sendFemalesIntoFight = false;
        this.underAttack = false;
        this.allowsRebonding = true; /* Does the tribe allow forming of new mating bonds in case one of the partners dies */
        this.findTargetTribe = (tribes) => {
            let suitableTargets = tribes.filter(x => x.fighters.length <= this.fighters.length);
    ...
    attack(tribes, log) {
        if (!this.underAttack && Math.random() <= this.aggressiveness) {
            let targetTribe = this.findTargetTribe(tribes);
            if (!targetTribe) {
                return;
            }
            targetTribe.underAttack = true;
            let attackLog = `Tribe '${this.name}' (${this.fighters.length} fighters) has attacked village '${targetTribe.name}' (${targetTribe.fighters.length} fighters).`;
            let casualties = 0;
            let targetCasualties = 0;
            let thisCasualtiesBeforeRetreat = this.universe.casualtiesBeforeRetreat * this.fighters.length;
            let targetCasualtiesBeforeRetreat = this.universe.casualtiesBeforeRetreat * targetTribe.fighters.length;
...
class Individual {
    constructor(tribe) {
        this.alive = true;
        this.findTargetIndividual = (individuals) => {
            if (individuals.length === 0) {
                return null;
            }
            return randomItem(individuals);
        };
        this.probabilityToSurvive = () => {
            let a, p, i = 0;
            do {
        ...
        this.findMateIndividual = () => {
            let suitableMates = this.tribe.individuals.filter(x => x.sex !== this.sex && !x.isPregnant);
    ...
    attack(individuals) {
        let targetIndividual = this.findTargetIndividual(individuals);
        if (!targetIndividual) {
            return;
        }
        let outcome = randomInteger(1, 3); // 1 - wins this individual, 2 draw, 3 wins targetIndividual
        switch (outcome) {
            case 1:
                targetIndividual.alive = false;
                break;
            case 2:
                this.alive = false;
                break;
    ...
    mate(log) {
        ...
        if (!this.hasMatingPartner) {
            let mate = this.findMateIndividual();
            if (mate != null) {
                ...
                let newBorn = new Individual(this.tribe);
                newBorn.age = 0;
...
jquery__WEBPACK_IMPORTED_MODULE_0__(document).ready(() => {
    let numberOfHeartbeats = 30;
    let selectedHeartbeat = 0;
    jquery__WEBPACK_IMPORTED_MODULE_0__("#divSlider").slider({
        range: "max",
        min: 0,
    ...
    let universe;
    function randomTribe(numberOfIndividuals) {
        let tribe = new Tribe(universe);
        tribe.individuals = [];
    ...
    function newSimulation() {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0__("#divGraph").empty();
        jquery__WEBPACK_IMPORTED_MODULE_0__("#divLog").empty();
        let divInitialUniverse = drawUniverse(universe);
        divInitialUniverse.find("h3").text("Initial state");
        jquery__WEBPACK_IMPORTED_MODULE_0__("#divGraph").append(divInitialUniverse.attr("id", getHeartbeatDivId(0)));
        for (let i = 1; i <= numberOfHeartbeats; i++) {
            let log = new Log();
            log.events.push(`HEARTBEAT ${i}`);
            universe.heartbeat(log);
            let divUniverse = drawUniverse(universe);
            divUniverse.find("h3").text("Heartbeat " + i);
            jquery__WEBPACK_IMPORTED_MODULE_0__("#divGraph").append(divUniverse.attr("id", getHeartbeatDivId(i)).hide());
        }
    }
    function drawIndividual(individual) {
        ...
        if (individual.age < individual.universe.matingAgeBegin) {
            divIndividual = jquery__WEBPACK_IMPORTED_MODULE_0__("<i class='fa'>&#xf1ae;</i>");
    ...
    function drawTribe(tribe) {
        let divTribe = jquery__WEBPACK_IMPORTED_MODULE_0__(`<div id='divTribe${tribe.name}'></div>`);
    ...
    function drawUniverse(universe) {
        let divUniverse = jquery__WEBPACK_IMPORTED_MODULE_0__(`<div />`);
    ...
    function getHeartbeatDivId(heartbeat) {
        return "divHeartbeat" + heartbeat;
    }
    newSimulation();
    jquery__WEBPACK_IMPORTED_MODULE_0__("#btnRun").click(() => {
        jquery__WEBPACK_IMPORTED_MODULE_0__("#btnRun").hide();
        let interval = window.setInterval(() => {
            let divCurrent = jquery__WEBPACK_IMPORTED_MODULE_0__("#" + getHeartbeatDivId(selectedHeartbeat));
            if (selectedHeartbeat < numberOfHeartbeats) {
                selectedHeartbeat++;
                let divNext = jquery__WEBPACK_IMPORTED_MODULE_0__("#" + getHeartbeatDivId(selectedHeartbeat));
                divCurrent.toggle();
                divNext.toggle();
                jquery__WEBPACK_IMPORTED_MODULE_0__("#divSlider").slider("option", "value", selectedHeartbeat);
            }
            else {
                jquery__WEBPACK_IMPORTED_MODULE_0__("#btnRun").show();
                clearInterval(interval);
            }
        }, 500);
    });
    jquery__WEBPACK_IMPORTED_MODULE_0__("#btnNew").click(() => {
        selectedHeartbeat = 0;
        newSimulation();
        jquery__WEBPACK_IMPORTED_MODULE_0__("#btnRun").show();
        jquery__WEBPACK_IMPORTED_MODULE_0__("#divSlider").slider("option", "value", selectedHeartbeat);
    });
    jquery__WEBPACK_IMPORTED_MODULE_0__("#divSlider").on("slide", (event, ui) => {
        let divCurrent = jquery__WEBPACK_IMPORTED_MODULE_0__("#" + getHeartbeatDivId(selectedHeartbeat));
        let sliderValue = ui.value;
        let divNext = jquery__WEBPACK_IMPORTED_MODULE_0__("#" + getHeartbeatDivId(sliderValue));
        divCurrent.toggle();
        divNext.toggle();
